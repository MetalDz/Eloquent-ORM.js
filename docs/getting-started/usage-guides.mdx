...
- loaded instance methods such as `fill()`, `save()`, and `patch()`
  - update an already loaded model instance
- low-level by-id methods such as `new User().update(id, data)` and `new User().delete(id)`
...
Use `create()` when you want to insert a brand new model in one call.
...
Use `update(id, data)` when you already know the primary key and want a direct service-layer write.
...
Use `delete(id)` for direct deletion by primary key.
...
Restore is available only when the model exposes soft-delete behavior.
