...
```ts
const created = await new Post().create({ title: "Hello ORM JS" });
const found = await Post.findOneBy("title", "Hello ORM JS");

if (found) {
  found.fill({ title: "Hello ORM JS Updated" });
  await found.save();
}

await new Post().delete(created?.id ?? 1);
```

5. If the model uses soft deletes, restore instead of recreating.

```ts
const model = new Post() as Post & { restore?: (id: number | string) => Promise<void> };
