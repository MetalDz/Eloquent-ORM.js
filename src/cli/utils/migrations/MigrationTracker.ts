  ...
  for (const row of hydratedRows) {
    ...
    if (!resolved) {
      ...
      if (createTableName) {
        ...
        if (exists) {
          throw new Error(
            `Applied migration "${row.name}" is missing from disk and table "${createTableName}" still exists. ` +
              `Restore the migration file, or run migrate:fresh for this connection, ` +
              `or manually drop "${createTableName}" and remove the migration row from "migrations".`
          );
        }
      }
